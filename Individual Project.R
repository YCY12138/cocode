...
summary(tobit1)
result1 <- survreg(Surv(LogHelpful,LogHelpful>0,type='left') ~ RatingMean+LogPeriod+factor(Mobile)+factor(Area)+factor(Season)+factor(Weekday), dist='gaussian',data = mydata)
summary(result1)
...
summary(tobit2)
result2 <- survreg(Surv(LogHelpful,LogHelpful>0,type='left') ~ factor(Local)+LogExperienced+Expertise+Rating+TitleLength+Length+I(Length^2)+Sentiment+I(Sentiment^2)+Subjectivity+Photo+RatingMean+LogPeriod+factor(Mobile)+factor(Area)+factor(Season)+factor(Weekday), dist='gaussian',data = mydata)
summary(result2)
...
summary(tobit3)
result3 <- survreg(Surv(LogHelpful,LogHelpful>0,type='left') ~ factor(Local)*Rating+LogExperienced+Expertise+Rating+TitleLength+Length+I(Length^2)+Sentiment+I(Sentiment^2)+Subjectivity+Photo+I(Rating_mc*LogExperienced_mc)+I(Rating_mc*Expertise_mc)+I(Rating_mc*TitleLength_mc)+I(Rating_mc*Length_mc)+I(Rating_mc*Length_mc^2)+I(Rating_mc*Sentiment_mc)+I(Rating_mc*Sentiment_mc^2)+I(Rating_mc*Subjectivity_mc)+I(Rating_mc*Photo_mc)+RatingMean+LogPeriod+factor(Mobile)+factor(Area)+factor(Season)+factor(Weekday), dist='gaussian',data = mydata)
summary(result3)
